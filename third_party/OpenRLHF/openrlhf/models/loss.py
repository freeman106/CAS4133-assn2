...
class SimPOLoss(nn.Module):
    """
    SimPO Loss (Meng et al., 2024, https://arxiv.org/abs/2405.14734)

    Reference-free preference optimization with length-normalized policy log-probabilities
    and a target reward margin gamma:

        L = - log sigmoid(
                (beta / |y+|) * sum_t log pi(y+_t | x, y+_<t)
              - (beta / |y-|) * sum_t log pi(y-_t | x, y-_<t)
              - gamma
            )

    Compared with DPO, SimPO removes the reference model entirely (half the memory of
    DPO at train time) and uses the average log-prob per response token, which empirically
    matches the inference-time generation objective more directly.

    The caller is expected to pass already-length-normalized policy logps (i.e. mean log-prob
    per response token). This mirrors how the SimPOTrainer slices ``_get_batch_logps`` output
    and keeps this loss module agnostic to padding / prompt-masking details.
    """

    def __init__(self, beta: float, gamma: float = 0.0) -> None:
        super().__init__()
        self.beta = beta
        self.gamma = gamma
    ...
    def forward(
        self,
        policy_chosen_logps_norm: torch.Tensor,
        policy_rejected_logps_norm: torch.Tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        pi_logratios = policy_chosen_logps_norm - policy_rejected_logps_norm
        logits = self.beta * pi_logratios - self.gamma
        loss = -F.logsigmoid(logits).mean()

        chosen_rewards = (self.beta * policy_chosen_logps_norm).detach()
        rejected_rewards = (self.beta * policy_rejected_logps_norm).detach()
        return loss, chosen_rewards, rejected_rewards
