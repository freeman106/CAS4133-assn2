...
class SimPOTrainer(ABC):
    ...
    Differences from DPOTrainer:
      * No reference model. SimPO is reference-free; halves the memory footprint at training
        time (no second model forward pass, no ref offload housekeeping).
      * Length-normalized log probabilities. Instead of summing log p(y|x) over response
        tokens, we use the per-token average. This matches the inference-time likelihood
        the model uses for generation and prevents the loss from being dominated by long
        completions.
      * Target reward margin gamma. The SimPO loss subtracts a constant gamma from the logit
        before sigmoid, encouraging a margin between the (length-normalized) chosen and
        rejected rewards rather than only their ordering.

    Args:
        model: policy model to be trained.
        strategy: DeepSpeed strategy wrapper (same one DPOTrainer uses).
        tokenizer: tokenizer.
        optim: optimizer.
        train_dataloader / eval_dataloader: dataloaders (DPO-style preference data).
        scheduler: lr scheduler.
        max_norm: gradient clipping max norm.
        beta: SimPO beta (typical 2.0 ~ 2.5, larger than DPO due to length normalization).
        gamma: target reward margin (typical 0.5 ~ 1.5; 0 disables margin).
        max_epochs: number of training epochs.
        save_hf_ckpt: also save HuggingFace-format weights.
        disable_ds_ckpt: skip DeepSpeed-format ckpt (used only for resume).
    """
    ...
    def __init__(
        ...
        self.beta = beta
        self.gamma = gamma
        self.loss_fn = SimPOLoss(self.beta, self.gamma)

        # Mixtral 8*7b
        self.aux_loss = self.args.model.aux_loss_coef > 1e-8

        # NLL loss (kept for parity with DPOTrainer)
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                # length-normalized logps (key SimPO change vs. DPO's summed logps)
                chosen_logps_norm, rejected_logps_norm, aux_loss, nll_loss = self.concatenated_forward(
                    self.model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens
                )

                # SimPO loss — no reference model
                preference_loss, chosen_reward, reject_reward = self.loss_fn(
                    chosen_logps_norm, rejected_logps_norm
                )
                # mixtral
                ...
                logs_dict = {
                    ...
                    "reward_margin": (chosen_reward - reject_reward).mean().item(),
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                chosen_logps_norm, rejected_logps_norm, aux_loss, _ = self.concatenated_forward(
                    self.model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens
                )

                loss, chosen_reward, reject_reward = self.loss_fn(chosen_logps_norm, rejected_logps_norm)
    ...
    def concatenated_forward(self, model, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens):
        """Forward the model once on the concatenated (chosen, rejected) batch and return
        per-sequence *length-normalized* log-probabilities for both halves.

        Returns: (chosen_logps_norm, rejected_logps_norm, aux_loss, nll_loss)
        """
        ...
        # _get_batch_logps returns BOTH summed and mean log-probs over response tokens.
        # SimPO uses the *mean* (length-normalized) variant.
        all_logps_sum, all_logps_mean = self._get_batch_logps(
            log_probs, att_masks, prompt_id_lens, average_log_prob=False
        )
        chosen_logps_norm = all_logps_mean[: chosen_ids.shape[0]]
        rejected_logps_norm = all_logps_mean[chosen_ids.shape[0] :]
        aux_loss = output.aux_loss if "aux_loss" in output else []
        return chosen_logps_norm, rejected_logps_norm, aux_loss, -all_logps_mean[: chosen_ids.shape[0]].mean()

    def concatenated_inputs(self, chosen_ids, c_mask, reject_ids, r_mask, prompt_id_lens):
        """Concatenate chosen + rejected into one batch to avoid two forward passes
        (faster especially under FSDP/ZeRO). Identical to DPOTrainer.concatenated_inputs."""
    ...
    def _get_batch_logps(
        ...
        """Compute per-sequence log-probabilities over response tokens.

        Returns: (sum_logp, mean_logp). SimPO uses mean_logp; sum_logp is returned for
        parity with DPOTrainer in case external code wants both.
        """
        ...
        logprobs_means = (per_token_logps * loss_masks).sum(-1) / loss_masks.sum(-1).clamp_min(1)
